...
const SOURCE_SITE_URL        = (process.env.SOURCE_SITE_URL || 'https://rivaltasulmincio.com').replace(/\/$/, '');
...
// ── Scraping HTML (fallback per siti non-WordPress) ──────────────────────────
...
async function scrapePostPage(url) {
  ...
  return { title, excerpt, content, date, link: url, imageUrl };
}
...
async function main() {
  ...
  if (isWp) {
    ...
    log('  ✗ WordPress non rilevato → uso scraping HTML');
    mode = 'html-scrape';
