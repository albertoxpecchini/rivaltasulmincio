...
async function handleAiAutofill(req, res) {
  ...
  if (!GEMINI_KEY && !ANTHROPIC_KEY) {
    ...
    res.end(JSON.stringify({ error: 'Chiave API AI non configurata. Imposta GEMINI_API_KEY (gratuita) o ANTHROPIC_API_KEY nelle variabili d\'ambiente Vercel.' }));
    return;
  }
...
Usa null per i campi non trovati nel testo/immagine. Non inventare informazioni non presenti. Rispondi SOLO con il JSON.`;
  ...
  // Extract JSON even if the model wraps it in a backtick code fence
