  ...
  <style>
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    body::before {
      ...
      background: repeating-conic-gradient(rgba(0,0,0,.015) 0% 25%, transparent 0% 50%) 0 0 / 4px 4px;
    ...
    .user-btn:hover { border-color: #aaa; }
    .user-avatar {
    ...
    .user-dropdown {
      ...
      min-width: 210px; z-index: 200; padding: 6px;
    }
    .user-dropdown.open { display: block; }
    .user-dropdown__title {
      padding: 8px 10px 6px; font-size: .67rem; font-weight: 700;
      text-transform: uppercase; letter-spacing: .07em; color: #72808a;
    }
    .user-dropdown a,
    ...
    .user-dropdown a.active {
      color: #1F6F8B; font-weight: 700;
      background: rgba(31,111,139,.07);
    }
    .user-dropdown a.item-primary {
      color: #fff; font-weight: 600;
      background: linear-gradient(135deg, #1F6F8B, #3a8c8a);
      margin-top: 2px;
    }
    .user-dropdown a.item-primary:hover { opacity: .9; }
    .user-dropdown__sep { height: 1px; background: var(--border); margin: 6px 4px; }
    .user-dropdown .danger { color: #b94747; }
    ...
    /* ── Bento grid (2-col, like the example) ── */
    .bento {
      display: grid;
    ...
    .card-hero::before {
      ...
      background: linear-gradient(90deg, var(--accent), var(--accent-2), var(--green), var(--accent-2), var(--accent));
    ...
    .avatar {
      ...
      display: flex; align-items: center; justify-content: center;
      font-size: 2.2rem; background: #e0eff4;
    }
    ...
    .hero-username {
      ...
      color: var(--text); letter-spacing: -.02em; line-height: 1.1; margin-bottom: 3px;
    }

    .hero-displayname {
      font-size: 13px; color: var(--muted); margin-bottom: 12px; font-weight: 500;
    }
    ...
    .chip-admin {
      color: #9a6700; border-color: rgba(249,168,37,.35);
      background: rgba(249,168,37,.08);
    }

    .hero-bio { font-size: 13px; color: var(--muted); line-height: 1.55; max-width: 320px; }
    ...
    .stat-label { font-size: 12px; color: var(--muted); font-weight: 500; line-height: 1.3; }
    ...
    .btn {
      ...
      padding: 8px 14px; cursor: pointer; text-decoration: none; transition: all .15s;
    }
    .btn:hover { border-color: #aaa; }
    .btn-primary {
      border-color: transparent;
      background: linear-gradient(135deg, var(--accent), var(--accent-2));
      color: #fff; box-shadow: 0 4px 16px rgba(31,111,139,.25);
    }
    .btn-primary:hover { box-shadow: 0 6px 22px rgba(31,111,139,.4); }
    ...
    .posts-count-tag { font-size: 12px; color: var(--muted); font-weight: 500; }
    ...
    .pcard:hover { box-shadow: 0 10px 28px rgba(0,0,0,.10); transform: translateY(-3px); }

    .pcard-img { height: 130px; overflow: hidden; position: relative; background: var(--bg); }
    ...
    .pcard-cat {
      ...
      border: 1px solid rgba(0,0,0,.06); color: var(--text);
    }

    .pcard-body { padding: 14px 16px; }
    .pcard-meta { font-size: 11px; color: var(--muted); margin-bottom: 6px; font-weight: 500; }

    .pcard-title {
      font-family: 'Sora', sans-serif; font-size: 14px; font-weight: 700;
      color: var(--text); line-height: 1.35; margin-bottom: 8px; letter-spacing: -.01em;
    }
    .pcard-title a { color: var(--text); transition: color .15s; }
    ...
    .pcard-foot { padding-top: 10px; border-top: 1px solid rgba(0,0,0,.05); }

    .btn-read {
      display: inline-flex; align-items: center; gap: 5px;
      font-family: 'DM Sans', sans-serif; font-size: 12px; font-weight: 700;
      color: var(--accent); text-decoration: none; transition: gap .15s;
    }
...
<div class="page">
  ...
  <div class="bento">
    ...
    <div class="card card-articles">
      <div class="num skel" id="stat-posts" style="font-size:36px;color:var(--accent);width:48px;height:38px"></div>
      <div class="stat-label">Articoli pubblicati</div>
    </div>

    <!-- Member since -->
    <div class="card card-since">
      <div class="num skel" id="stat-since" style="font-size:20px;color:var(--accent-2);width:90px;height:24px"></div>
...
(async function () {
    ...
    wrap.innerHTML = `
      <div class="user-menu" id="userMenuWrap">
        <button class="user-btn" id="userMenuBtn" aria-haspopup="true" aria-expanded="false">
          <span class="user-avatar" style="background:${color}">${emoji}</span>
          @${safe}
        </button>
  ...
  if (pe || !prof) {
    document.querySelector('.bento').innerHTML = '<div class="card" style="grid-column:1/-1;padding:60px;text-align:center;color:#86868b">Profilo non trovato.</div>';
    return;
  }
  ...
  av.style.background = prof.avatar_color || '#1F6F8B';
...
<div style="line-height:0;background:var(--bg)"><svg viewBox="0 0 1440 60" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:60px"><path d="M0,20 C360,55 1080,0 1440,30 L1440,60 L0,60 Z" fill="#0F2A30"/></svg></div>
<footer style="background:#0F2A30;color:#EAF4F4;font-family:'DM Sans',sans-serif">
  <div style="max-width:1180px;margin:0 auto;padding:48px 22px 0">
    <div style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:48px;padding-bottom:40px;border-bottom:1px solid rgba(234,244,244,.1)">
      <div>
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px">
          <div style="width:46px;height:46px;background:linear-gradient(135deg,#1F6F8B,#2E7D32);border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:1.4rem">🌿</div>
          <div>
            <div style="font-size:1.1rem;font-weight:600">Pro Loco Rivalta sul Mincio</div>
            <div style="font-size:.7rem;color:rgba(234,244,244,.5);text-transform:uppercase;letter-spacing:.08em">Ecoturismo · Mantova</div>
          </div>
        </div>
        <p style="font-size:.9rem;color:rgba(234,244,244,.7);line-height:1.8;max-width:300px">Associazione di promozione turistica, culturale e ambientale del borgo di Rivalta sul Mincio.</p>
        <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:14px">
          <span style="font-size:.72rem;padding:4px 11px;border:1px solid rgba(234,244,244,.15);border-radius:20px;color:rgba(234,244,244,.6)">🌿 Ramsar 1971</span>
          <span style="font-size:.72rem;padding:4px 11px;border:1px solid rgba(234,244,244,.15);border-radius:20px;color:rgba(234,244,244,.6)">🇪🇺 Natura 2000</span>
          <span style="font-size:.72rem;padding:4px 11px;border:1px solid rgba(234,244,244,.15);border-radius:20px;color:rgba(234,244,244,.6)">🐟 PAT 2013</span>
        </div>
      </div>
      <div>
      ...
      <div>
        <h4 style="font-size:.95rem;font-weight:600;color:#fff;margin-bottom:16px;padding-bottom:10px;border-bottom:2px solid #1F6F8B">Informazioni</h4>
        <ul style="list-style:none;display:flex;flex-direction:column;gap:8px">
          <li><a href="#" style="font-size:.88rem;color:rgba(234,244,244,.6);text-decoration:none">Privacy Policy</a></li>
          <li><a href="#" style="font-size:.88rem;color:rgba(234,244,244,.6);text-decoration:none">Cookie Policy</a></li>
          <li><a href="#" style="font-size:.88rem;color:rgba(234,244,244,.6);text-decoration:none">Diventa socio</a></li>
          <li><a href="#" style="font-size:.88rem;color:rgba(234,244,244,.6);text-decoration:none">Note legali</a></li>
        </ul>
      </div>
      <div>
        <h4 style="font-size:.95rem;font-weight:600;color:#fff;margin-bottom:16px;padding-bottom:10px;border-bottom:2px solid #1F6F8B">Contatti</h4>
        <div style="display:flex;flex-direction:column;gap:10px;font-size:.86rem;color:rgba(234,244,244,.65)">
          <div>📍 Via Porto, 31<br>Rivalta sul Mincio (MN)</div>
          <div>✉️ <a href="mailto:info@prolocorivalta.mn.it" style="color:rgba(234,244,244,.75);text-decoration:none">info@prolocorivalta.mn.it</a></div>
          <div>📞 <a href="tel:+393398995680" style="color:rgba(234,244,244,.75);text-decoration:none">+39 339 899 5680</a></div>
        </div>
      </div>
    </div>
    <div style="padding:24px 0;display:flex;align-items:center;justify-content:space-between;font-size:.8rem;color:rgba(234,244,244,.45);flex-wrap:wrap;gap:10px">
