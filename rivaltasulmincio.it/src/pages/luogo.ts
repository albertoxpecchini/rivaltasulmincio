...
export function render({ params }: PageContext): PageResult | null {
  ...
  <header class="place__header">
    <p class="place__kind">${categoryMark(place.category)} <span class="label">${categoryLabel(place.category)}</span> · ${place.kind}${
  ...
  <div class="place__layout">
    <dl class="facts">
      ...
      <div><dt>Orari</dt><dd>${place.openingHours ? html`<span class="tabular">${place.openingHours}</span>` : 'Orario non disponibile'}</dd></div>
      <div><dt>Coordinate</dt><dd class="tabular">${formatCoordinates(place)}</dd></div>
    </dl>
